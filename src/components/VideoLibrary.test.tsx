...
import VideoLibrary, {
  VideoItem,
  VideoLibraryProps,
} from "./VideoLibrary/VideoLibrary";
...
window.electronAPI = {
  ...
  openVideoFileDialog: jest.fn(() =>
    Promise.resolve({
      filePath: "mock/path/to/imported_video.mp4",
      canceled: false,
    }),
  ),
...
describe("VideoLibrary", () => {
  ...
  it('calls onOpenRecorder when "Ouvrir Enregistreur" button is clicked', () => {
    render(<VideoLibrary {...defaultProps} />);
    fireEvent.click(
      screen.getByRole("button", { name: /Ouvrir Enregistreur/i }),
    );
    jest.runAllTimers();
  ...
  it('calls onImportVideo when "Importer une vidéo" button is clicked', () => {
    render(<VideoLibrary {...defaultProps} />);
    fireEvent.click(
      screen.getByRole("button", { name: /Importer une vidéo/i }),
    );
    jest.runAllTimers();
  ...
  it('calls onToggleSelectMode when "Activer Multi-sélection" button is clicked', () => {
    render(<VideoLibrary {...defaultProps} />);
    fireEvent.click(
      screen.getByRole("button", { name: /Activer Multi-sélection/i }),
    );
    jest.runAllTimers();
  ...
  it('calls onShareSelected when "Partager la sélection" button is clicked in multi-select mode', () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /Partager la sélection/i }),
    );
